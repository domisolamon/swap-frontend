...
const CreateStake = ({onChnageName, onChangeAmount, isDisabled, onCreateStake,mode,balance,title="Create a Stake", ...props}) => {
    ...
    const createStake = async() => {
        if(mode === Irrevisible && Number(amount) < 100000000) {
            setAmount(100000000)
            toast.warn("You should deposit more than 100,000,000 tokens in this mode.", {
                toastId: "account",
            })
            return;
        }
        ...
        // if(!await canWithdrawPrimaryToken(amount)) {
        //     if(! toast.isActive("cantwithdraw!"))
        //         toast.warn("Insufficient balance in Pool!",{
        //             toastId: "Insufficient balance in Pool!",
        //         })
        //     return;
        // }
        ...
        switch (mode) {
            ...
            case Irrevisible: statement1 = "Creating Node"; statement2 = "Created"; break;
    ...
    return <Box display="flex" flexDirection={"column"} border="1px solid rgba(0,0,0,.125)" bgcolor="white" {...props}>
        ...
        <Box display="flex" p="1.25rem">
            <Button variant="contained" color="success" disabled={isDisabled} /* disabled={true} */  onClick={createStake}>Confirm</Button>
        </Box>
    </Box>
}
