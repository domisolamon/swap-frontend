...
import { borrow, downgradeNFT, fractionlize, getTierInfo, getTokenName, updateNFT, updateURI, redeem, canUpgrade, canDowngrade, canFraction, canUpdateURI, canBorrow, canRedeem, canDoRoyalty, transfer } from "../../hooks/useTokenInfo";
import { Borrow, Downgrade, Fractionalize, UpdateURI, Upgrade, ModeList, Holding, Grace, Extra } from "../../pages/Manage";
import { ExpToNumber, numberWithCommas } from "../../utils/calculation";
...
const NFTFractionCard = ({
    ...
    const _downGrade = async()=>{
        ...
        const fee = priceList[tokenType] * royaltiFee
        const price = flatFee
        const tier0From = Math.floor(maxTier0 / maxRegular[tokenType])
        const tier0To = Math.floor(maxTier0 / maxRegular[selectedTier])
    ...
    const _fractionalize = async() => {
        const fee = priceList[tokenType] * royaltiFee
        const price = flatFee
        const fractionAmount = Math.floor(maxTier0 / maxRegular[tokenType]);
    ...
    const _borrow = async() => {
        let amount = Math.floor(maxTier0 / maxRegular[tokenType] * borrowFee / 100)
    ...
    const isOverflowOfBalance = (tierId) => {
        ...
        for(let i = 1; i <= 10; i++) {
            total += Math.floor(maxTier0 / maxRegular[11-i]) * userInfo[i]?.res.length;
            maxLimit += Math.floor(maxTier0 / maxRegular[11-i]) * maxWallet[11-i]
        }
        total += Math.floor(maxTier0 / maxRegular[tierId])
        ...
        switch(mode) {
            case Upgrade:
                return <Box display="flex" justifyContent="space-between" width="100%" mb="5px">
                    ...
                    <Button variant="contained" color="info" size="small" onClick={_upgradeNFT} sx={{textTransform: 'none'}}>{ModeList[mode]}</Button>
                </Box>
            case Downgrade:
                return <Box display="flex" justifyContent="space-between" width="100%" mb="5px">
                    <NativeSelect
                    ...
                    <Button variant="contained" color="warning" size="small" onClick={_downGrade} sx={{textTransform: 'none'}}>{ModeList[mode]}</Button>
                </Box>
            case Fractionalize:
                return <Box display="flex" width="100%" mb="5px" justifyContent="center">
                    <Button variant="contained" color="error" size="small" onClick={_fractionalize} sx={{textTransform: 'none'}}>{ModeList[mode]}</Button>
                </Box>
            case UpdateURI:
                return <Box display="flex" justifyContent="space-between" width="100%" mb="5px">
                    <CustomInput style={{width: '150px', height: "30px", marginRight: "10px"}} value={uri} onChange={(e)=>{setUri(e.target.value)}} />
                <Button variant="contained" color="warning" size="small" onClick={_updateURI} sx={{textTransform: 'none'}}>{ModeList[mode]}</Button>
            </Box>
            case Borrow:
                if(tokenType === 10) return;
                const receive = Math.floor(maxTier0 / maxRegular[tokenType] * borrowFee / 100);
                return  <Box display="flex" flexDirection="column">
                            ...
                            <Box display="flex" justifyContent="center">
                                <Button variant="contained" color="success" size="small" disabled={poolFraction < receive} onClick={_borrow} sx={{textTransform: 'none'}}>Borrow</Button>
                            </Box>
                </Box>
            case Holding:
            case Grace:
            case Extra:
                return <Box display="flex" flexDirection="column">
                            ...
                            <Box display="flex" justifyContent="center" color="#ff64dd">
                                <Button variant="outlined" color="inherit" size="small" onClick={_redeem} sx={{textTransform: 'none'}}>Redeem</Button>
                            </Box>
                </Box>
            default:
                return null;
    ...
    const transferNFT = async() => {
        if(!account || !transferAddress) return;
        const fee = priceList[tokenType] * royaltiFee
        await toast.promise(transfer(account, transferAddress, tokenId, currency, fee, royaltyOption), {
    ...
    const getRedeemAmount = () => {
        let amount = tokenType && maxRegular && maxRegular[tokenType] ? Math.floor(maxTier0 / maxRegular[tokenType]) : 0;
    ...
    <NFTMangeComponent style={{transform: toggle ? 'translateY(0)' : 'translateY(100%)'}}>
        ...
        <Box display="flex" flexDirection="column" justifyContent="space-between" width="100%">
            ...
            { mode < Borrow && <Box display="flex" justifyContent="space-between" width="100%" mb="10px" color="#ff64dd">
                <CustomInput style={{width: '150px', height: "30px", marginRight: "10px"}} placeholder="Transfer To" value={transferAddress} onChange={(e)=>{setTransferAddress(e.target.value)}} />
                <Button variant="contained" color="secondary" sx={{textTransform: 'none'}} size="small" onClick={transferNFT}>Transfer</Button>
            </Box>}
